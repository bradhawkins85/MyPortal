...
async def eligible_staff(
    company_id: int, job_title: str | None = None
) -> list[dict[str, Any]]:
    """Return only unambiguous, explicitly linked, active portal identities."""
    key = normalize_job_title(job_title or "")
    ...
    return [
        row
        for row in rows
        if normalize_job_title(str(row.get("job_title") or "")) == key
    ]
...
async def create(
    *,
    ...
    if item is None:
        return None
    title_key = (
        normalize_job_title(job_title or "") if selector_type == "job_title" else None
    )
    ...
    grant_id = await db.execute_returning_lastrowid(
        ...
        (
            ...
            job_title,
            title_key,
            int("enumerate" in capabilities),
            int("reveal" in capabilities),
            int("share" in capabilities),
            int("administer" in capabilities),
            purpose,
            grantor_user_id,
            approver_user_id,
            expires_at,
...
async def get(grant_id: int) -> dict[str, Any] | None:
    return await db.fetch_one(
        "SELECT * FROM credential_standing_grants WHERE id = %s", (grant_id,)
    )


async def list_for_credential(
    company_id: int, credential_id: int
) -> list[dict[str, Any]]:
...
async def resolve(
    user_id: int, company_id: int, capability: Capability
) -> list[dict[str, Any]]:
    ...
    result = []
    for row in rows:
        if row["selector_type"] == "job_title" and normalize_job_title(
            str(row.get("job_title") or "")
        ) != row.get("job_title_key"):
        ...
        if row["selector_type"] == "job_title":
            staff = await db.fetch_one(
                "SELECT job_title FROM staff WHERE company_id = %s AND portal_user_id = %s AND enabled = 1",
                (company_id, user_id),
            )
            if (
                not staff
                or normalize_job_title(str(staff.get("job_title") or ""))
                != row["job_title_key"]
            ):
                continue
        result.append(row)
...
async def reveal(
    user_id: int, company_id: int, credential_id: int
) -> tuple[dict[str, Any], str] | None:
    matches = [
        row
        for row in await resolve(user_id, company_id, "reveal")
        if int(row["credential_id"]) == credential_id
    ]
    if not matches:
        return None
...
async def revoke(
    grant_id: int, company_id: int, actor_user_id: int
) -> dict[str, Any] | None:
    changed = await db.execute_rowcount(
