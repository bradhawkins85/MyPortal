...
async def improve_product_description(product_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        if status not in {"skipped", "error", "failed"}:
            ...
            response_ai_used = bool(description_html)
    except ValueError:
        pass
    ...
    log_info("Product description refreshed", product_id=product_id, feature_count=len(features), ai_used=bool(response_ai_used))
