(function () {
  ...
  function bindDeletionRules() {
    ...
    document.querySelector('[data-retention-form]')?.addEventListener('submit', async (event) => { event.preventDefault(); const target = event.currentTarget; await requestJson('/scheduler/webhook-retention', { method: 'PUT', body: JSON.stringify({ enabled: target.elements.enabled.checked, retentionDays: Number(target.elements.retentionDays.value) }) }); window.location.reload(); });
