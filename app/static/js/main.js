(function () {
  ...
  function setupPullToRefresh() {
    ...
    const indicator = document.querySelector('[data-pull-to-refresh]');
    const label = indicator ? indicator.querySelector('[data-pull-to-refresh-label]') : null;
    const iconEl = indicator ? indicator.querySelector('.pull-to-refresh__icon') : null;

    if (!content || !indicator || !label) {
      return;
    }
    ...
    function updateIndicator(distance) {
      const capped = Math.min(distance, MAX_PULL);
      const progress = Math.min(distance / THRESHOLD, 1);
      const height = capped * 0.5;

      indicator.style.height = height + 'px';

      if (iconEl) {
        iconEl.style.transform = 'rotate(' + (progress * 360) + 'deg)';
      }
