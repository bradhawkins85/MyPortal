...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"company_id": 4, "requester_id": None, "requester_staff_id": 9},
            False,
            None,
        ),
    ],
...
@pytest.mark.anyio
async def test_api_allows_technician_reply_for_staff_requester(monkeypatch):
    class ReplyReached(Exception):
        pass
    ...
    monkeypatch.setattr(
        ticket_api.tickets_repo, "get_ticket", lambda _ticket_id: _async(ticket)
    )
    monkeypatch.setattr(
        ticket_api.tickets_repo,
        "create_reply",
        lambda **_kwargs: _raise_async(ReplyReached),
    )

    with pytest.raises(ReplyReached):
...
async def test_admin_rejects_internal_note_without_company(monkeypatch):
    ...
    assert rendered["reply_error"] == "Set a Company before adding an internal note."
    assert rendered["reply_body"] == "Internal"
