...
from app.services.m365 import (
    _COMPLIANCE_ADMIN_ROLE_TEMPLATE_ID,
    _GRAPH_APP_ID,
...
async def test_try_grant_missing_permissions_assigns_exchange_admin_role():
    """Required EXO/Teams/compliance directory roles are assigned during connect flow."""
    ...
    assert len(role_assignment_urls) == 3, "Exchange, Teams, and Compliance Admin roles should be assigned"
    ...
    assert _COMPLIANCE_ADMIN_ROLE_TEMPLATE_ID in assigned_role_def_ids, "Compliance Admin role must be assigned"
...
@pytest.mark.anyio("asyncio")
async def test_ensure_compliance_admin_role_assigns_when_missing():
    """Compliance Administrator directory role is assigned when not present."""
    posted: list[dict] = []
    ...
    with (
        ...
        result = await m365_service._ensure_compliance_admin_role(
            access_token="admin-token",
            sp_object_id="sp-123",
        )
    ...
    assert len(posted) == 1
    assert posted[0]["principalId"] == "sp-123"
    assert posted[0]["roleDefinitionId"] == _COMPLIANCE_ADMIN_ROLE_TEMPLATE_ID
    assert posted[0]["directoryScopeId"] == "/"


@pytest.mark.anyio("asyncio")
async def test_ensure_compliance_admin_role_noop_when_already_assigned():
    """Returns False without POST when Compliance Administrator role is already assigned."""
    posted: list[dict] = []
...
@pytest.mark.anyio("asyncio")
async def test_ensure_compliance_admin_role_logs_on_failure():
    """Returns False and logs error when Compliance Administrator assignment fails."""
