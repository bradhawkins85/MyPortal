...
| API Keys & Audit Trails | API key issuance/usage logging and audit log viewer exposed via REST and admin pages.【F:src/server.ts†L92-L104】【F:src/views/admin.ejs†L1-L60】 | FastAPI now exposes hashed API key creation, revocation, usage telemetry, and audit logging with Swagger-managed endpoints.【F:app/api/routes/api_keys.py†L1-L123】【F:app/repositories/api_keys.py†L1-L190】 | Build complementary UI panels for inline API key rotation and cross-service audit correlations.
