...
| --- | --- | --- | --- |
| Authentication & Sessions | Redis-backed Express sessions, CSRF middleware, login throttling, trusted devices, and TOTP enrolment during login and admin management.【F:src/server.ts†L1060-L1119】【F:src/server.ts†L1502-L1619】 | No authentication routes, session storage, CSRF, or MFA support are implemented.【F:app/api/routes/users.py†L1-L58】【F:app/main.py†L49-L61】 | Build session management, CSRF protection, rate limiting, password reset, and TOTP flows in FastAPI.
...
| API Keys & Audit Trails | API key issuance/usage logging and audit log viewer exposed via REST and admin pages.【F:src/server.ts†L92-L104】【F:src/views/admin.ejs†L1-L60】 | No API key storage or auditing in Python rewrite.【F:app/repositories/users.py†L1-L64】【F:app/templates/dashboard.html†L1-L21】 | Implement API key entities, hashed storage, usage logging, and admin reporting.
